...
void idMenuScreen_HUD::ShowScreen( const mainMenuTransition_t transitionType )
{
    ...
    gazeCursor = root.GetNestedSprite( "_center", "crosshairGaze" );
...
void idMenuScreen_HUD::UpdateCursorState()
{
    ...
    if( !cursorTalking && !cursorInCombat && !cursorGrabber && !cursorItem && !gazeCursor )
    {
        ...
        if( gazeCursor )
        {
            gazeCursor->StopFrame( 1 );
            gazeCursor->SetVisible( false );
        }
        ...
        else if( cursorInCombat )
        {
            ...
            else if( cursorState != CURSOR_IN_COMBAT )
            {
                ...
                if( combatCursor )
                {
                    ...
                    // TODO: Ben - make the gaze cursor visible here
